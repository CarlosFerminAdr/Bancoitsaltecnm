...
class RegistroAelectronicaController extends Controller
{
    ...
    {
        $registro = Registro::where(['carrera_id' => 4, 'status' => 1])->paginate();
        view('registroElectronicaAuto.pdf' ,compact('registro'))->with('i', (request()->input('page', 1) - 1) * $registro->perPage());

        $pdf = PDF::loadView('registroElectronicaAuto.pdf',compact('registro'));
