...
class QuimidisponibleController extends Controller
{
    ...
    public function edit(Proyecto $quimicaon)
    {
        $estados = Estado::all();
        $empresas = Empresa::all();
        $periodos = Periodo::all();
        $carreras = Carrera::all();
        return view('quimicaon.edit',compact('quimicaon', 'estados', 'empresas', 'periodos', 'carreras'));
    }

    public function update(UpdateProyectoRequest $request, Proyecto $quimicaon)
    {
        $quimicaon->update([
        ...
        if($request->nombre){
            $quimicaon->proyectograma()->update([
                'nombre'=> $request->nombre,
                'nalumnos'=> $request->nalumnos,
                'flimite'=> $request->flimite
            ]);
        }else{
            return redirect('quimicaon.edit');
        }

        return redirect('quimicaon')->with('mensaje','Proyecto Autorizado Correctamente!');
    }
